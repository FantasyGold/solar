...
import (
	...
	"github.com/fantasygold/solar/contract"
	"github.com/fantasygold/solar/deployer"
	"github.com/fantasygold/solar/deployer/eth"
	"github.com/fantasygold/solar/deployer/fantasygold"
	"github.com/fantasygold/solar/varstr"
	kingpin "gopkg.in/alecthomas/kingpin.v2"
)

var (
	app               = kingpin.New("solar", "Solidity smart contract deployment management.")
	fantasygoldRPC           = app.Flag("fantasygold_rpc", "RPC provider url").Envar("FGC_RPC").String()
	fantasygoldSenderAddress = app.Flag("fantasygold_sender", "(fantasygold) Sender UTXO Address").Envar("FGC_SENDER").String()
...
var (
	errorUnspecifiedRPC = errors.New("Please specify RPC url by setting FGC_RPC or ETH_RPC or using flag --fantasygold_rpc or --eth_rpc")
)

func (c *solarCLI) RPCPlatform() RPCPlatform {
	if *fantasygoldRPC == "" && *ethRPC == "" {
		log.Fatalln(errorUnspecifiedRPC)
	}

	if *fantasygoldRPC != "" {
		return RPCFantasyGold
	}
...
func (c *solarCLI) FantasyGoldRPC() *fantasygold.RPC {
	rpc, err := fantasygold.NewRPC(*fantasygoldRPC)
	if err != nil {
		fmt.Println("Invalid FGC RPC URL:", *fantasygoldRPC)
		os.Exit(1)
	}
...
func (c *solarCLI) Deployer() (deployer deployer.Deployer) {
	...
	if rawurl := *fantasygoldRPC; rawurl != "" {

		rpcURL, err = url.ParseRequestURI(rawurl)
		if err != nil {
			log.Fatalf("Invalid RPC url: %#v", rawurl)
		}
		deployer, err = fantasygold.NewDeployer(rpcURL, c.ContractsRepository(), *fantasygoldSenderAddress)
	}
